     ...
     <div class="card-header">
        ...
        <div>
            @auth
                ...
                <form method="POST" action="{{route('discussions.best-reply',['discussion'=>$discussion->slug,'reply'=>$reply->id])}}">
                    @csrf
                    <button type="submit" class="btn btn-sm btn-info">お気に入りに追加</button>
                </form>
                @endif
            @endauth
        </div>
...
@else
  <a href="{{route('login')}}" class="btn btn-info">入力するにはログインが必要です</a>
@endauth
    </div>
</div>
